...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the NORT and zNORT money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 60151, 51474));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, northern, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all Northern specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:60151"));
    ...
    strUsage += HelpMessageOpt("-backupznort=<n>", strprintf(_("Enable automatic wallet backups triggered after each zNORT minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizenorthernamount=<n>     " + strprintf(_("Keep N NORT anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) %i The Northern Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
            ...
            try {
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() > Params().Zerocoin_StartHeight()) {
                        RecalculateZNORTMinted();
                        RecalculateZNORTSpent();
                    }
                    RecalculateNORTSupply(1);
                }
                ...
                // Northern: recalculate Accumulator Checkpoints that failed to database properly
...
// ********************************************************* Step 8: load wallet
    ...
    if (fDisableWallet) {
        ...
        bool fEnableZNORTBackups = GetBoolArg("-backupznort", true);
        pwalletMain->setZNORTAutoBackups(fEnableZNORTBackups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeNorthernAmount = GetArg("-anonymizenorthernamount", 0);
//    if (nAnonymizeNorthernAmount > 999999) nAnonymizeNorthernAmount = 999999;
//    if (nAnonymizeNorthernAmount < 2) nAnonymizeNorthernAmount = 2;
    ...
    LogPrintf("Anonymize Northern Amount %d\n", nAnonymizeNorthernAmount);
    ...
    /* Denominations
       ...
       For example:
       1NORT+1000 == (.1NORT+100)*10
       10NORT+10000 == (1NORT+1000)*10
    */
