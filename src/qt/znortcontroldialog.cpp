...
#include "ui_znortcontroldialog.h"
...
std::list<std::string> ZNORTControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZNORTControlDialog::listMints;

ZNORTControlDialog::ZNORTControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZNORTControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZNORTControlDialog::~ZNORTControlDialog()
{
    delete ui;
}

void ZNORTControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZNORTControlDialog::updateList()
{
...
void ZNORTControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZNORTControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZNORT_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZNORTControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZNORTControlDialog::GetSelectedMints()
{
...
void ZNORTControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
