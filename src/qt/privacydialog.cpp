                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zNORT ought to be enough for anybody." - Bill Gates, 2017
    ui->zNORTpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzNORTSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzNORT->setEnabled(false);
        ui->pushButtonMintzNORT->setToolTip(tr("zNORT is currently disabled due to maintenance."));

        ui->pushButtonSpendzNORT->setEnabled(false);
        ui->pushButtonSpendzNORT->setToolTip(tr("zNORT is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zNORTpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzNORT_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNORT is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zNORT...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zNORT in ") + 
...
void PrivacyDialog::on_pushButtonSpendzNORT_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zNORT is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zNORT
        sendzNORT();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zNORT
    sendzNORT();
}

void PrivacyDialog::on_pushButtonZNORTControl_clicked()
{
    ZNORTControlDialog* zNORTControl = new ZNORTControlDialog(this);
    zNORTControl->setModel(walletModel);
    zNORTControl->exec();
}

void PrivacyDialog::setZNORTControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzNORTSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzNORT()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Northern Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zNORTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zNORT is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " NORT </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zNORT</b>";
    ...
    // use mints from zNORT selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZNORTControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZNORTControlDialog::GetSelectedMints();
    }
    ...
    // Clear znort selector in case it was used
    ZNORTControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zNORT Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " NORT, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zNORT Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zNORT </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zNORT "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zNORT "));
    ui->labelzNORTAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzNORTSyncStatus->setVisible(fShow);
}
